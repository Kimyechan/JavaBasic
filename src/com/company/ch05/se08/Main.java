 ...
 * - Depreacted: 앞으로 사용되지 않을 클래스/ 메소드/ 변수 ... 사용하지 말라는 의미
...
// @Target @Retention : 메타 애노테이션
    // @Target : 애노테이션을 사용할 수 있는 대상을 설정
        ...
        // PACKAGE : 패키지
    // Retention : 애노테이션 정보를 어디까지 유지할 것인가를 결정하는 Policy
        // SOURCE : 컴파일러가 사용  .java -> .class로 변경하는 과정에서 쓰임 .class파일에 포함 x
...
/**
 * 맴버 변수를 대상으로 하는, Reflection API에서 쓸 수 있는 애노테이션
...
class AnnotationUsage {
    ...
    // value가 String[]이므로 value=String[]로 넣어
    @MyAnnotation(value={"server"}, valueTwo = "localhost")
    String serverIp;

    // value가 String[]이지만, 길이가 1이면 String만 넣어줘도 됨
