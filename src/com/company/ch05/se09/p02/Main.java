package com.company.ch05.se09.p02;

/**
 * 팩토리 패턴
 * - 구상 클래스 객체(추상 클래스를 구현한 것; Concrete class)를 전담하여 생성하는 클래스를 구햔하는 패턴
 */
interface Shape {
    void draw();
}
...
public class Main {
    public static void main(String[] args) {
        ...
        // 새로운 구상 클래스가 만들어졌을 때 코드 수정이 불가피
        // 클래스가 구상 클래스에 의존하게 됨
        // 클래스는 추상 클래스(인터페이스)에 의존하는 것이 더 바람직하다.
        if(shapeType.equals("Circle")) {
            shape = new Circle();
        } else if (shapeType.equals("Rectangle")) {
            shape = new Rectangle();
