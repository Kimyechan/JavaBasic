 ...
 * - 제네릭 클래스로 되어있어, 다양한 객체를 요소(Element)로 담을 수 있다.
...
public class Main {
    public static void main(String[] args) {
        ...
        intList.add(2, 678493); // 중간 index에 add할 경우 자료를 뒤로 한칸 씩 민다.
        System.out.println(intList);
        System.out.println(intList.size());
        ...
        // remove() 매소드는 index에서 obj를 제거하고 그것을 반환
        ...
        // listIterator를 이용한 접근환 - foreach문은 사 이것을 짧게 쓴 것
        ...
        // List -> Set으로 한벅씩만 옮기면 중복이 제거됨
        // 1중 for문을 이용해서 비교할 필요 없이 알아서 삭제됨 O(n)

        NavigableSet<String> stringSet2 = new TreeSet<>(); // 이진 탐색 트리 구조
        ...
        System.out.println(map.put("ABCDE", 1023)); // 5반환하고 1023으로 설정
        ...
        System.out.println(map4.ceilingKey("b"));
        System.out.println(map4.ceilingEntry("b").getValue());

        System.out.println(map4);

        System.out.println(map4.pollFirstEntry().getValue());
        System.out.println(map4.pollFirstEntry().getValue());
        System.out.println(map4.pollFirstEntry().getValue());
        System.out.println(map4.pollFirstEntry().getValue());
        ...
        Properties prop = new Properties(); // Hashtable<String, String> 싱속
        prop = System.getProperties(); // System의 property가 이 형식을 이용한다.
